...
last_commit: 77c309b
yandex_account_id: 62946101
---

# Kayak Moscow — Ads Automation

Внутренний сервис для автоматизации рекламы Kayak Moscow в Яндекс Директе. Не SaaS, один аккаунт `kayakmoscow@yandex.ru` (ID `62946101`). Оператор: ИП Рыбников И.Г. (ИНН 772861688006).

> **Точка входа для AI-агента в новой сессии:** [[_AI_BRIEF]].
> **Сводка сессии 2026-06-08:** [[Session_2026-06-08]] — что разобрали, что замутировали в проде, что запланировали.
> **Roadmap сезона 2026:** [[Roadmap_2026]].
> **Аудит Yandex от 2026-06-06:** [[Audit_Direkta_2026-06-06]] — индекс качества **6.6/10**, 11 actionable рекомендаций.
> **Полный срез по Direct API на 2026-06-08:** [[Direct_Live_State_2026-06-08]] — что реально в Директе vs YAML vs PDF.
> **Disaster recovery:** [[SYSTEM_snapshot_2026-05-14]] (источник правды — `docs/SYSTEM.md` в репо).

## Текущее состояние (2026-06-08, источник: Direct API live pull)

- **7 активных кампаний.** За 10 мая — 7 июня: **92 297 ₽ / 10 225 кликов / 442 конверсии / средний CPA 208 ₽**. PDF-аудит Yandex показал 110 конверсий — он считает только цель оптимизации, реальная картина по всем целям Метрики в 4× лучше.
- **Наши ЕПК работают:**
  - `709824114` Поиск-Бренд → 15 874 ₽, **80 конверсий, CPA 198 ₽** (search CTR 7.0%).
  - `709824286` Поиск-Generic → 5 728 ₽, **58 конверсий, CPA 99 ₽** (search CTR 3.9%) — лучший CPA из ЕПК.
  - `709893520` РСЯ-Wide State=ON, **3 объявления с видео и баннерами** уже залиты, но 8 показов / 0 конверсий за период — PAY_FOR_CONVERSION не учится без конверсий. Тупик.
  - `709824342` Погодный-Эксп → **SUSPENDED**, успел открутить 7 032 ₽ / 31 конв.
  - `709902809` Погодный-Контроль (создан Codex 2026-05-14) → **SUSPENDED**, 142 клика / 2 конв.
- **3 ТК на автопилоте Yandex AI** дают 71% расхода и 39% конверсий, через API недоступны:
  - `88607049` ТК \| Каяки \| АТ \| Sh → 53 524 ₽ / 148 конв / CPA **362 ₽**.
  - `88656583` ТК \| Каяки \| РН \| Sh → 2 600 ₽ / 11 конв / CPA 236 ₽.
  - `709892014` Оптимизация ТК РН → 1 250 ₽ / 12 конв / CPA **104 ₽** (лучший в аккаунте).
- **P0-план 2026-05-14 от Codex почти полностью выполнен:** 5 SitelinkSet (по 4 БС), 12 callouts (все ACCEPTED+Associated), Я.Бизнес карточка (BusinessId 69769476747), сплит Generic на 2 группы (каяк/SUP), A/B-вторые объявления в Бренд/Generic, парная контрольная Погодный-Контроль создана.

## Сделано 2026-06-08 (через API)

- `ENABLE_SITE_MONITORING=YES` на всех 5 наших кампаниях.
- Метрика-счётчик `49922815` привязан к Погодный-Контроль `709902809` (был `CounterIds=null`).
- Удалена DRAFT-группа `5757096553` «Москва-Красногорск Тест1» в РСЯ-Wide (вместе с audience target + autotargeting-ключом).
- Удалены 6 SUP-дублей из AG-1 Generic (5749931733) — SUP-семантика теперь только в AG-2 (5751744960).
- **Бид-модификаторы на Бренд + Generic** (Campaign level): возраст 0-17=−100%, 25-34=+15%, 35-44=+15%, Москва-город (213)=+15%. Time Targeting: будни 18-21=+20%, выходные 10-21=+20%.
- РСЯ-Wide `709893520` владелец только что включил — пусть покрутится 7-14 дней.
- Погодный-Эксп `709824342` остановлен **намеренно** — владелец вернётся к парному тесту позже.

## Что осталось (P1)

1. **Mobile bid modifier −15%** на Бренд + Generic нужно вшивать в weather curve в `ads_campaign_strategy` (правка кода) — Campaign-level через API не работает из-за hourly bid loop, который перепишет AdGroup-уровень каждые 30 минут.
2. **Через 7-14 дней проверить РСЯ-Wide.** Если PAY_FOR_CONVERSION не заводится — временно сменить на AVERAGE_CPC 15 ₽ для разогрева.

Полный план: [[Direct_Live_State_2026-06-08#Приоритизированные действия]].
...
## Планы новых кампаний (draft, ждут запуска)

- [[Plan_TG_Channel_2026-06-08]] — **ЕПК (UnifiedCampaign)** на рост подписчиков `@kayakmoscow` через Telegram-каналы и Max. Стратегия `WB_MAXIMUM_CLICKS`, BidCeiling 60 ₽, бюджет 7 500 ₽/нед, KPI: ≥300 подписчиков за 90 дней, CPS ≤ 330 ₽.
- [[Plan_TG_Site_2026-06-08]] — **ЕПК** на бронь через сайт kayakmoscow.ru через Telegram-каналы и Max. Стратегия `WB_MAXIMUM_CLICKS`, BidCeiling 70 ₽, бюджет 11 000 ₽/нед, KPI: ≥50 конверсий за 90 дней, CPA ≤ 1 500 ₽.

Совокупно — 40% + 60% от 240 000 ₽ ассигнованных на «Telegram через Я.Директ» по [[Исследование рекламного продвижения Kayak Moscow на сезон 2026]].

**API создания скелета подтверждён 2026-06-08** живой пробой через `v501/campaigns/add` с `AdvertisedItem.Href`. Codex может создать скелет ЕПК, но «Места показа → Telegram + Max», категории каналов, загрузка изображений и выбор CTA-кнопки — только UI владельцем (тумблер в Settings API отсутствует).

## Связанные заметки

- [[_AI_BRIEF]] — **точка входа для AI-агента**
- [[Audit_Direkta_2026-06-06]] — официальный аудит Yandex от 2026-06-06, индекс 6.6/10
