...
source_commit: 77c309b + локальные правки 2026-05-14
---

# SYSTEM snapshot 2026-05-14 — Kayak Moscow Ads Automation

> Снапшот `docs/SYSTEM.md` на 2026-05-14 после получения прод-доступа Direct API, создания 3 живых кампаний и обновления документации.
> Заменяет [[SYSTEM_snapshot_2026-05-08]] (тот делался до prod switch).

# SYSTEM.md — полное описание Kayak Moscow Ads Automation

Дата актуальности: 2026-05-14 (последний коммит `77c309b`).
...
- 3 кампании созданы в Direct prod 2026-05-12: 709824114 (Бренд), 709824286 (Generic), 709824342 (Погодный-Эксп.).
- Все на модерации, ждём активации + баланса + включения через UI.
- `OPENAI_API_KEY` ещё не задан → `daily_negatives_job` молчит.
- `hourly_bid.enabled` в `ads_runtime_settings` = false до активации кампаний.
...
7. **`DailyBudget` совместим только с ручными стратегиями** (по факту prod-аккаунта 2026-05-12). При `AVERAGE_CPC`, `AVERAGE_CPA`, `WB_MAXIMUM_CLICKS` и прочих автостратегиях `Campaigns.add` отвергает с `code 6000 "Дневной бюджет можно использовать только совместно с ручными стратегиями"`. Контроль расхода — только через `WeeklySpendLimit` в `BiddingStrategy.Search`. В нашем `UnifiedCampaignInput` поле `daily_budget_amount` теперь опциональное, передавать только для ручных стратегий.
8. **Title/Title2/Text — узкий whitelist символов** (Yandex `code 5002 "Используются недопустимые символы"`). Принимаются только латиница/кириллица (RU + TR + UA + BY + KZ алфавиты)/цифры/знаки пунктуации. Запрещены: `★ • ✓ → ⭐ ♥ ♦ ☆ ⚡` и большинство unicode-символов вне базового. Решение: `scripts/create_campaigns_2026.py` имеет pre-flight `check_unicode(plan)` который ловит нарушения до отправки.
9. **`code 1000 "Сервис временно недоступен"`** — спорадическая 500 от Yandex. Не считается ошибкой данных, нужен retry с экспоненциальным backoff. Реализовано в `with_retry(...)` в `scripts/create_campaigns_2026.py`.
10. **`AdGroups.update` для NegativeKeywords — replace, не append**. Передаваемый `NegativeKeywords.Items` полностью заменяет существующий список. Для добавления одной минус-фразы нужен read-modify-write: `AdGroups.get(field_names=[..., "NegativeKeywords"])` → склеить → `update`.
...
1. ✅ ~~Прод-доступ к Direct API не получен~~ — одобрено 2026-05-08, `DIRECT_API_ENV=prod` на VDS, 3 кампании созданы.
2. ✅ ~~AdGroups в sandbox «фантомные»~~ — в prod работают штатно.
...
5. **Push минус-фразы в Direct не реализован.** Approval flow в `/dashboard/negatives` помечает `decision=approved` в БД, но `Campaigns.update`/`Keywords.add` ещё не вызывает. Запланировано после ≥2 недель работы прод-кампаний.
6. **«Товарные кампании» (ТК) не доступны через Direct API** — даже по прямому Id. Управляются только UI Direct'a. 3 активные ТК у нас автопилотятся Yandex AI, не трогаем.
7. **РСЯ и Ретаргетинг через API не делаем** — нужны креативы (баннеры/видео) и Аудитории Метрики, через UI Direct'a.
8. **n8n-ads контейнер запущен без workflow'ов**. БД `kayak_ads_n8n` создаётся при первом старте.
9. **Directus коллекции `ads_*` не созданы**. M3 закрыт — наш UI заменил.
10. **Бэкап БД** делается через ERP-механизм, отдельных скриптов не написано.
11. **monitoring/alerting** — только logs+digest. Нет внешнего uptime-монитора.
12. **CI/CD** — нет GitHub Actions. Деплой ручной (`git pull` + `docker compose build` + `up -d`).
13. **Координаты Медузы** в `scripts/calibrate_weather_curve.py` приблизительные. Влияние на калибровку малое.

---

## 18. Roadmap указатели

См. `docs/roadmap.md` + полный roadmap в Obsidian `[[Roadmap_2026]]`. Где мы на 2026-05-14:

- ✅ M1 Sandbox foundation
- ✅ M2 Weather loop + reports + калибровка
- ⏸ M3 Directus config — закрыт нашим UI
- ✅ M4 Dashboard + OAuth + **прод-доступ Direct API одобрен 2026-05-08**
- ✅ M5 Minus-words + approval flow (pipeline + UI; push в Direct отложен)
- ✅ Runtime UI (2026-05-07)
- ✅ M5.1 Анализ архивов + 5 YAML-пресетов + 3 декларации кампаний (2026-05-09..12)
- 🟢 **M6 Production switch — в процессе**. 3 кампании в Direct: 709824114 / 709824286 / 709824342. Ждём модерацию + баланс + включение через UI.
- ⏳ M6.1 РСЯ + Ретаргетинг руками в UI Direct
- ⏳ M6.2 Push минус-фраз в Direct
- ⏳ M6.3 Первая ревизия (конец июня)
- ⏳ M7 Audiences + сезонный режим (июль)
- ⏳ M8 VK Ads (июль)
- ⏳ M9 Offline conversions через Tinkoff (сентябрь-октябрь)
...
2. **Включить кампании** (State=ON в Direct UI).
3. **Включить `hourly_bid.enabled`** в `/dashboard/settings`.
4. **Положить `OPENAI_API_KEY`** в `/root/kayak-ads/deploy/.env` → перезапустить engine → `daily_negatives.enabled=true`.
5. **Whitelist маркетолога и директора продаж** в `DASHBOARD_ALLOWED_YANDEX_LOGINS`.
